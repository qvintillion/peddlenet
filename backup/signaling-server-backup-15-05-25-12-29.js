...
function cleanupStaleP2PConnections() {
  ...
  let cleaned = 0;
  for (const staleSocketId of staleConnections) {
    const connectionData = p2pConnections.get(staleSocketId);
    if (connectionData && connectionData.status === 'connected') {
      // Reduce the active connection count
      meshMetrics.activeP2PConnections = Math.max(0, meshMetrics.activeP2PConnections - 1);
    }
    p2pConnections.delete(staleSocketId);
    cleaned++;
  }
  
  if (cleaned > 0) {
    console.log(`🧹 Cleaned up ${cleaned} stale P2P connections`);
    
    // Log the cleanup activity
    addActivityLog('p2p-cleanup', {
      staleConnections: cleaned,
      remainingConnections: p2pConnections.size,
      activeUsers: activeSocketIds.size
    }, '🧹');
  }
  
  // Also clean up orphaned peer references within existing connections
  for (const [socketId, connectionData] of p2pConnections.entries()) {
    const originalPeerCount = connectionData.peers.size;
    // Remove any peer references that don't correspond to active sockets
    connectionData.peers = new Set(Array.from(connectionData.peers).filter(peerId => activeSocketIds.has(peerId)));
    
    const cleanedPeerCount = connectionData.peers.size;
    if (cleanedPeerCount !== originalPeerCount) {
      console.log(`🧹 Cleaned ${originalPeerCount - cleanedPeerCount} stale peer references for socket ${socketId}`);
    }
    
    // If connection has no peers left, remove it entirely
    if (connectionData.peers.size === 0 && connectionData.status === 'connected') {
      p2pConnections.delete(socketId);
      meshMetrics.activeP2PConnections = Math.max(0, meshMetrics.activeP2PConnections - 1);
      console.log(`🧹 Removed empty P2P connection for socket ${socketId}`);
    }
  }
}
...
app.get('/admin/mesh-status', requireAdminAuth, (req, res) => {
  try {
    ...
    const enhancedMeshMetrics = {
      ...
      // 🔧 ENSURE: Never return null metrics and use actual counts
...
io.on('connection', (socket) => {
  ...
  // 🚀 CUSTOM WEBRTC: Signaling handlers to replace PeerJS cloud service
  socket.on('webrtc-offer', ({ to, offer, fromPeerId, roomId }) => {
    try {
      console.log(`🔄 WebRTC offer relay: ${socket.id} -> ${to} in room ${roomId}`);
  ...
  socket.on('webrtc-answer', ({ to, answer, fromPeerId, roomId }) => {
    try {
      console.log(`🔄 WebRTC answer relay: ${socket.id} -> ${to} in room ${roomId}`);
  ...
  socket.on('webrtc-ice-candidate', ({ to, candidate, fromPeerId, roomId }) => {
    try {
      console.log(`❄️ ICE candidate relay: ${socket.id} -> ${to}`);
  ...
  socket.on('webrtc-connection-established', ({ to, fromPeerId, roomId }) => {
    try {
      console.log(`✅ WebRTC connection established: ${socket.id} <-> ${to} in room ${roomId}`);
      
      // Notify both peers
      socket.to(to).emit('webrtc-connection-established', {
        from: socket.id,
        fromPeerId,
        roomId,
        timestamp: Date.now()
      ...
      console.error('❌ WebRTC connection establishment error:', error);
    }
  });
  
  socket.on('webrtc-connection-failed', ({ to, error, fromPeerId, roomId }) => {
    try {
      console.log(`❌ WebRTC connection failed: ${socket.id} <-> ${to} - ${error}`);
      
      // Notify target peer
      socket.to(to).emit('webrtc-connection-failed', {
        from: socket.id,
        fromPeerId,
        error,
        roomId,
      ...
      console.error('❌ WebRTC connection failure handling error:', error);
    }
  });
  
  socket.on('webrtc-peer-disconnected', ({ to, fromPeerId, roomId }) => {
    try {
      console.log(`👋 WebRTC peer disconnected: ${socket.id} -> ${to}`);
      
      // Notify target peer
      socket.to(to).emit('webrtc-peer-disconnected', {
        from: socket.id,
        fromPeerId,
        roomId,
        timestamp: Date.now()
      ...
      // Log activity
      addActivityLog('webrtc-peer-disconnected', {
        peer1: socket.id,
        peer2: to,
        roomId,
        fromPeerId
      ...
      console.error('❌ WebRTC peer disconnection error:', error);
    }
  });
  
  // WebRTC room discovery - get list of peers available for connection
  socket.on('webrtc-discover-peers', ({ roomId }) => {
    try {
      if (!rooms.has(roomId)) {
        socket.emit('webrtc-peers-discovered', { peers: [], roomId });
        return;
      }
      ...
      console.log(`🔍 WebRTC peer discovery: ${availablePeers.length} peers found in room ${roomId}`);
      
      socket.emit('webrtc-peers-discovered', {
      ...
      console.error('❌ WebRTC peer discovery error:', error);
      socket.emit('webrtc-error', { type: 'discovery-failed', error: error.message });
    }
  });
  ...
  socket.on('disconnect', () => {
    try {
      ...
      if (p2pConnections.has(socket.id)) {
        ...
        // Update metrics - only count this connection once
        ...
        console.log(`🌐 Enhanced cleanup of P2P connections for ${socket.id}`);
      }
      ...
      // 🔧 FINAL CLEANUP: Run a cleanup pass to ensure consistency
      setTimeout(() => {
        cleanupStaleP2PConnections();
      }, 1000); // Run cleanup 1 second after disconnect to catch any remaining stale connections
    } catch (error) {
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('🧹 Periodic P2P cleanup scheduled every 30 seconds');
});

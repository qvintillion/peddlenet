...
// Enhanced clickable metric card component with active/total display
...
function MetricCard({ title, activeValue, totalValue, extraInfo, icon, color, onClick, isClickable }: MetricCardProps) {
  const colorClasses = {
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          {/* Active Count - Large and prominent */}
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-3xl font-bold text-white">{activeValue}</span>
            <span className="text-lg font-semibold text-green-400">active</span>
          </div>
          
          {/* Total Count - Smaller, secondary */}
          <div className="flex items-center gap-1 mt-1">
            <span className="text-sm text-gray-400">of</span>
            <span className="text-lg font-semibold text-gray-300">{totalValue}</span>
            <span className="text-sm text-gray-400">total</span>
          </div>
          
          {extraInfo && (
            <p className="text-gray-500 text-xs mt-2">{extraInfo}</p>
          )}
        </div>
      ...
      {isClickable && (
        <div className="mt-3 text-center">
          <span className="text-xs text-purple-400 font-medium">📊 Click for details</span>
        </div>
      )}
    </div>
  );
...
// Simple metric card for non-active/total metrics
...
function SimpleMetricCard({ title, value, subvalue, icon, color, onClick, isClickable }: SimpleMetricCardProps) {
  const colorClasses = {
    green: 'border-green-500/50 bg-green-500/10',
    yellow: 'border-yellow-500/50 bg-yellow-500/10',
    red: 'border-red-500/50 bg-red-500/10',
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-purple-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <MetricCard
            title="Users"
            activeValue={displayData.realTimeStats.activeUsers || 0}
            totalValue={displayData.realTimeStats.totalUsers || 0}
            extraInfo={`Peak connections: ${displayData.realTimeStats.peakConnections || 0}`}
            icon="👥"
            color={(displayData.realTimeStats.activeUsers || 0) > 10 ? 'green' : (displayData.realTimeStats.activeUsers || 0) > 5 ? 'yellow' : 'gray'}
            onClick={() => setShowUserDetails(true)}
            isClickable={true}
          />
          <MetricCard
            title="Rooms"
            activeValue={displayData.realTimeStats.activeRooms || 0}
            totalValue={displayData.realTimeStats.totalRooms || 0}
            extraInfo="Active chat rooms with users"
            icon="🏠"
            color={(displayData.realTimeStats.activeRooms || 0) > 5 ? 'green' : (displayData.realTimeStats.activeRooms || 0) > 2 ? 'yellow' : 'gray'}
            onClick={() => setShowRoomDetails(true)}
            isClickable={true}
          />
          <SimpleMetricCard
            title="Messages/Min"
            value={(displayData.realTimeStats.messagesPerMinute || 0).toString()}
            subvalue={`Total: ${(displayData.realTimeStats.totalMessages || 0).toLocaleString()}`}
            icon="💬"
            color={(displayData.realTimeStats.messagesPerMinute || 0) > 20 ? 'green' : (displayData.realTimeStats.messagesPerMinute || 0) > 10 ? 'yellow' : 'gray'}
          />
          <SimpleMetricCard
            title="Network Quality"
            value={`${displayData.networkStatus.quality || 100}%`}
            subvalue={`Latency: ${displayData.networkStatus.avgLatency || 0}ms`}
            icon="🌐"
            color={(displayData.networkStatus.quality || 100) > 90 ? 'green' : (displayData.networkStatus.quality || 100) > 70 ? 'yellow' : 'red'}
          />
        </div>
          ...
          <div className="bg-gray-800/80 rounded-lg p-6 backdrop-blur-sm border border-gray-700/50">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold">{displayData.realTimeStats.totalRooms}</span>
              </div>
              <div className="flex justify-between">

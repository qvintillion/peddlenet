...
// 🔧 FIXED: CORS configuration with Firebase domains
function getCorsOrigins() {
  ...
  // 🎯 CRITICAL FIX: Add Firebase staging domain
  origins.push("https://festival-chat-peddlenet.web.app");
  
  // 🎯 CRITICAL FIX: Add Firebase preview channel pattern
  // This is a wildcard pattern that Firebase uses for preview channels
  origins.push(/^https:\/\/festival-chat-peddlenet--[\w-]+\.web\.app$/);
  ...
  console.log('🔧 CORS origins configured:', origins.map(o => o.toString()));
  return origins;
}
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '1.0.0-minimal-fixed-cors',
    status: 'running',
    description: 'Fixed CORS for Firebase domains and admin dashboard',
    features: ['admin-dashboard-fixed', 'room-codes', 'websocket-signaling', 'cors-firebase-fixed'],
    environment: {
    ...
    cors: {
      allowedOrigins: getCorsOrigins().map(o => o.toString()),
      note: 'Firebase domains and preview channels supported'
    },
    endpoints: {
      health: '/health',
      signaling: '/socket.io/',
    ...
    fixes: [
      ...
      '🔧 FIXED CORS for Firebase domains',
      '🔧 FIXED CORS for preview channels'
    ],
    timestamp: Date.now()
  });
});
...
app.get('/health', (req, res) => {
  res.json({ 
    status: 'ok',
    service: 'PeddleNet Signaling Server',
    version: '1.0.0-minimal-cors-fixed',
    cors: 'Firebase domains supported',
    timestamp: Date.now()
  });
});
...
app.get('/signaling-proxy', (req, res) => {
  res.json({
    ...
    endpoint: '/socket.io/',
    version: '1.0.0-minimal-fixed-cors',
    features: ['peer-discovery', 'room-management', 'admin-dashboard', 'cors-firebase-fixed'],
    timestamp: Date.now()
  });
});

console.log('✅ Added room code endpoints with CORS fixes');
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(``);
  console.log(`🔧 CORS CONFIGURATION (FIXED FOR FIREBASE):`);
  console.log(`   ✅ localhost:3000 (development)`);
  console.log(`   ✅ https://festival-chat-peddlenet.web.app (Firebase staging)`);
  console.log(`   ✅ https://festival-chat-peddlenet--*.web.app (preview channels)`);
  console.log(`   ✅ https://peddlenet.app (Vercel production)`);
  console.log(``);
  ...
  console.log(``);
  console.log(`📊 ADMIN DASHBOARD ENDPOINTS (Fixed CORS errors):`);
  ...
  console.log(`   ✅ CORS errors for Firebase domains`);

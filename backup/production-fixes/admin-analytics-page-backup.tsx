...
// 🔧 PRODUCTION FIX: Environment-aware Login Form Component
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-purple-900 text-white flex items-center justify-center p-4">
      <div className="bg-gray-800/80 rounded-lg p-8 backdrop-blur-sm border border-gray-700/50 w-full max-w-md">
        ...
        {/* 🚨 PRODUCTION FIX: Only show default credentials in non-production */}
        {!isProduction() && (
...
export default function AdminAnalyticsPage() {
  ...
  // 🔧 FIXED: Helper function for API calls that connects to the correct server
  const makeAPICall = async (endpoint: string, options: RequestInit = {}) => {
    ...
    // 🔧 FIXED: Get WebSocket server URL from window object (injected at build time)
    ...
    if (!isLocal && wsServer) {
      ...
      console.log('🌐 Using WebSocket server for admin API:', `${apiServerUrl}/admin${endpoint}`);
      
      try {
        const response = await fetch(`${apiServerUrl}/admin${endpoint}`, {
          ...options,
          headers
        });
        ...
        console.error('❌ WebSocket server API call failed:', error);
        throw error;
      }
    }
    ...
    if (isLocal) {
      try {
        console.log('🌐 Trying local WebSocket server:', `${serverUrl}/admin${endpoint}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-purple-900 text-white">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mt-8 text-center text-gray-400 text-sm">
          ...
          <div className="mt-1">
            PeddleNet Admin Dashboard v4.5.0-production-ready-20250613-183500 • 

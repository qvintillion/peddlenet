...
# 🔥 P2P Admin Dashboard Connection Test - ENHANCED
...
if curl -s http://localhost:3001/health > /dev/null 2>&1; then
    ...
    response=$(curl -s -w "%{http_code}" -u "th3p3ddl3r:letsmakeatrade" http://localhost:3001/admin/mesh-status 2>/dev/null)
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "❌ Failed to connect to mesh endpoint"
    fi
...
if curl -s http://localhost:3000/api/health > /dev/null 2>&1; then
    ...
    # Test mesh API proxy
    response=$(curl -s -w "%{http_code}" -u "th3p3ddl3r:letsmakeatrade" http://localhost:3000/api/admin/mesh-status 2>/dev/null)
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "❌ Failed to connect to mesh API proxy"
    fi
...
echo "5. Use window.enableAdminP2PTesting() to enable P2P testing"
echo "6. Use window.testMeshEndpoint() to test the admin API directly"
echo "7. Use window.checkAdminP2PStatus() to check P2P system status"

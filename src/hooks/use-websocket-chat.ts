...
const getWebSocketServerUrl = () => {
  ...
  if (isLocalDev) {
    const url = `ws://localhost:3001`;
    console.log(`🏠 Local development WebSocket: ${url}`);
    return url;
  }
  ...
  if (isPrivateIP || port === '3000') {
    const url = `ws://${hostname}:3001`;
    console.log(`📱 Mobile development WebSocket: ${url}`);
    return url;
  }
  ...
  if (hostname.includes('firebase') || hostname.includes('web.app')) {
    console.log(`🔥 Firebase WebSocket: ${productionWebSocketUrl}`);
    return productionWebSocketUrl;
  }
  
  // Vercel production - use production WebSocket server
  if (hostname.includes('vercel.app') || hostname === 'peddlenet.app') {
    console.log(`▲ Vercel WebSocket: ${productionWebSocketUrl}`);
    return productionWebSocketUrl;
  }
  
  // Default fallback with warning
  const fallbackUrl = 'ws://localhost:3001'; // Changed to local for development
  console.warn(`⚠️ Using fallback WebSocket URL: ${fallbackUrl}`);
  return fallbackUrl;
};
...
export function useWebSocketChat(roomId: string, displayName: string) {
  ...
  const connectToServer = useCallback(async () => {
    ...
    if (connectionLockRef.current) {
      console.log(`🔒 Connection lock active, skipping attempt`);
      return;
    }
    
    if (now - lastConnectionAttempt.current < MIN_CONNECTION_INTERVAL) {
      console.log(`⏰ Too soon since last attempt (${now - lastConnectionAttempt.current}ms < ${MIN_CONNECTION_INTERVAL}ms)`);
      return;
    }
    ...
    if (!roomId || !effectiveDisplayName || isConnectingRef.current) {
      console.log(`⏸️ [${connectionId.current}] Skipping connection - missing requirements or already connecting`);
      return;
    }
    ...
    console.log(`🔒 Connection lock acquired, attempting connection...`);
    ...
    socket.on('disconnect', (reason) => {
      ...
      if (isCloudRunColdStart) {
        console.log(`❄️ [${connectionId.current}] Cloud Run cold start detected - will retry after delay`);
      ...
      if (shouldAutoReconnect && effectiveDisplayName && !reason.includes('server') && retryCount < 3) {
        ...
        console.log(`🔄 [${connectionId.current}] Auto-reconnect in ${backoffDelay}ms (attempt ${retryCount + 1}/3)`);
        ...
        console.log(`🛑 [${connectionId.current}] Auto-reconnect disabled or max retries reached`);
      }
    });
    ...
    socket.on('connect_error', (error) => {
      ...
      if (isFatalError) {
        console.error(`🚨 Fatal error detected, stopping retries: ${errorMessage}`);
      ...
      if (retryCount >= 2) {
        console.error(`🛑 Max retry attempts reached (${retryCount}), stopping completely`);
        setShouldAutoReconnect(false);
        setConnectionCooldown(false);
        return;
      }
      
      console.log(`🔄 Retrying connection in ${backoffDelay}ms (attempt ${retryCount + 1}/2)`);
    ...
    if (roomId && effectiveDisplayName && 
      ...
      setTimeout(async () => {
        ...
        if (!effectiveDisplayName || 
            ...
            connectionLockRef.current ||
            isConnectingRef.current) {
          console.log(`⏸️ Initialization conditions changed, skipping connection`);
          return;
        }
        ...
        try {
          ...
          if (response.ok) {
            console.log(`✅ Server health check passed, attempting single connection...`);
            connectToServer();
          } else {
            console.warn(`⚠️ Server health check failed:`, response.status);
          }
        } catch (error) {
          console.warn(`⚠️ Server health check error:`, error);
          console.log(`🛑 NOT attempting connection due to health check failure`);
        }
      }, 5000); // Much longer 5 second delay
    }

    return () => {
      console.log(`🧹 [${connectionId.current}] Cleanup - releasing all locks`);

...
try {
  globalWebRTCInstances = new Map();
} catch (error) {
  console.error('❌ Failed to initialize globalWebRTCInstances:', error);
  globalWebRTCInstances = {
    entries: () => [],
    get: () => undefined,
    set: () => {},
    delete: () => {},
...
// CRITICAL: Global rate limiter to prevent useEffect loops - PRODUCTION FIX v5
let globalInitAttempts;
try {
  globalInitAttempts = new Map();
} catch (error) {
  console.error('❌ Failed to initialize globalInitAttempts:', error);
  globalInitAttempts = {
    entries: () => [],
    get: () => 0,
    set: () => {},
...
export function useNativeWebRTC(roomId: string, displayName: string, disabled: boolean = false) {
  ...
  // CRITICAL: Check for concurrent instances - PRODUCTION FIX v5
  const checkConcurrentInstances = useCallback(() => {
    const instanceKey = `${roomId}-${displayName || 'anonymous'}`;
    const now = Date.now();
    
    // CRITICAL FIX: Null safety for Map operations
    if (!globalWebRTCInstances || typeof globalWebRTCInstances.entries !== 'function') {
      console.warn('⚠️ [WebRTC Hook] globalWebRTCInstances not properly initialized');
      return true; // Allow initialization if Map is broken
    }
    
    try {
      // CRITICAL FIX: Extra safety for entries() call and Array.from
      let entriesResult;
      try {
        entriesResult = globalWebRTCInstances.entries();
      } catch (error) {
        console.error('❌ [WebRTC Hook] Error calling globalWebRTCInstances.entries():', error);
        globalWebRTCInstances.clear();
        return true;
      }
      
      // CRITICAL FIX: Ensure entriesResult is iterable before Array.from
      if (!entriesResult || typeof entriesResult[Symbol.iterator] !== 'function') {
        console.warn('⚠️ [WebRTC Hook] Invalid entries iterator, clearing map');
        globalWebRTCInstances.clear();
        return true;
      }
      
      // Clean up old instances with extra null safety
      const entries = Array.from(entriesResult);
      for (const [key, instance] of entries) {
        if (instance && typeof instance.timestamp === 'number' && now - instance.timestamp > INSTANCE_TIMEOUT) {
          globalWebRTCInstances.delete(key);
        }
      }
    } catch (error) {
      console.error('❌ [WebRTC Hook] Error cleaning up instances:', error);
      // Clear the map and continue
      globalWebRTCInstances.clear();
      return true;
    }
  ...
  // Get WebSocket server URL based on environment - FIXED for desktop compatibility - CACHE BUST v2
  const getWebSocketURL = useCallback(() => {
    ...
    // 🔧 CRITICAL FIX: Use environment variable from build process
    ...
    // Fallback to production server for preview deployments
    if (hostname.includes('vercel.app') || hostname.includes('firebase')) {
      const productionUrl = 'wss://peddlenet-websocket-server-hfttiarlja-uc.a.run.app';
      console.log(`🌐 Using production WebSocket URL for preview: ${productionUrl}`);
      return productionUrl;
    }
    
    // Firebase hosting - use production server
  ...
  const updateStatus = useCallback(() => {
    // CRITICAL FIX: Add null safety for connectionsRef.current.values()
    let connectionsValues;
    try {
      connectionsValues = connectionsRef.current?.values();
    } catch (error) {
      console.error('❌ [UPDATE STATUS] Error accessing connections values:', error);
      connectionsValues = null;
    }
    
    const activeConnections = connectionsValues && typeof connectionsValues[Symbol.iterator] === 'function' 
      ? Array.from(connectionsValues).filter(conn => conn.dataChannel?.readyState === 'open').length
      : 0;
  ...
  // 🔥 ENHANCED: Force complete ICE restart for all connections - PRODUCTION FIX v4
  const forceICERestart = useCallback(() => {
    console.log('⚡ [ICE RESTART] Forcing ICE restart for all connections...');
    
    // CRITICAL FIX: Add comprehensive null checks for connectionsRef.current
    if (!connectionsRef.current || typeof connectionsRef.current.entries !== 'function') {
      console.log('📭 No connections ref available or invalid');
      return 'No connections ref';
    }
    
    // CRITICAL FIX: Extra safety for entries() call
    let entriesResult;
    try {
      entriesResult = connectionsRef.current.entries();
    } catch (error) {
      console.error('❌ [ICE RESTART] Error calling entries():', error);
      return 'Error accessing connections';
    }
    
    // CRITICAL FIX: Ensure entriesResult is iterable before Array.from
    if (!entriesResult || typeof entriesResult[Symbol.iterator] !== 'function') {
      console.log('📭 No valid entries iterator available');
      return 'No valid connections iterator';
    }
    
    const currentConnections = Array.from(entriesResult);
    if (!currentConnections || currentConnections.length === 0) {
      console.log('📭 No connections to restart');
      return 'No connections found';
    }
    
    currentConnections.forEach(([peerIdForRestart, conn]) => {
      console.log(`⚡ [ICE RESTART] Restarting ICE for ${peerIdForRestart} (current state: ${conn.peerConnection.iceConnectionState})`);
      try {
        conn.peerConnection.restartIce();
      } catch (error) {
        console.error(`❌ Failed to restart ICE for ${peerIdForRestart}:`, error);
      }
    });
  ...
  const sendMessage = useCallback((message: Omit<Message, 'id' | 'timestamp'>): string | null => {
    ...
    try {
      ...
      console.error('❌ [SEND MESSAGE] Error accessing connections values:', error);
      connectionsValues = null;
    }
    
    const activeConnections = connectionsValues && typeof connectionsValues[Symbol.iterator] === 'function'
      ? Array.from(connectionsValues).filter(conn => conn.dataChannel?.readyState === 'open')
      : [];
  ...
  useEffect(() => {
    ...
    // CRITICAL: Rate limiting to prevent useEffect loops - ENHANCED for admin dashboard v2
    const rateKey = `${roomId}-${displayName || 'anonymous'}`;
    const currentAttempts = globalInitAttempts.get(rateKey) || 0;
    
    // CRITICAL FIX: Clean up old rate limit entries with null safety
    try {
      let entriesResult;
      try {
        entriesResult = globalInitAttempts.entries();
      } catch (error) {
        console.error('❌ [RATE LIMIT] Error accessing globalInitAttempts:', error);
        globalInitAttempts.clear();
      }
      
      if (entriesResult && typeof entriesResult[Symbol.iterator] === 'function') {
        const entries = Array.from(entriesResult);
        for (const [key, timestamp] of entries) {
          if (typeof timestamp === 'number' && now - timestamp > RATE_LIMIT_WINDOW) {
            globalInitAttempts.delete(key);
          }
        }
      }
    } catch (error) {
      console.error('❌ [RATE LIMIT] Error cleaning up entries:', error);
      globalInitAttempts.clear();
    }
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.NativeWebRTCDebug = {
        getConnections: () => {
          try {
            const entriesIterator = connections?.entries();
            return entriesIterator && typeof entriesIterator[Symbol.iterator] === 'function'
              ? Array.from(entriesIterator)
              : [];
          } catch (error) {
            console.error('❌ [DEBUG GET CONNECTIONS] Error:', error);
            return [];
          }
        },
        ...
        testP2PWithAllPeers: () => {
          ...
          setTimeout(() => {
            ...
            try {
              const keysIterator = connections?.keys();
              currentConnections = keysIterator && typeof keysIterator[Symbol.iterator] === 'function'
                ? Array.from(keysIterator)
                : [];
            } catch (error) {
              console.error('❌ [P2P TEST] Error getting connections:', error);
            }
  ...
  return {
    ...
    status,
    roomPeers: (() => {
      // CRITICAL FIX: Add null safety for connections.keys()
      try {
        const keysIterator = connections?.keys();
        return keysIterator && typeof keysIterator[Symbol.iterator] === 'function' 
          ? Array.from(keysIterator) 
          : [];
      } catch (error) {
        console.error('❌ [ROOM PEERS] Error:', error);
        return [];
      }
    })(),
    connectToPeer,
    ...
    getConnectedPeers: () => {
      ...
      try {
        const keysIterator = connections?.keys();
        if (!keysIterator || typeof keysIterator[Symbol.iterator] !== 'function') {
          return [];
        }
        return Array.from(keysIterator).filter(peerId => {
          const conn = connections.get(peerId);
          return conn && conn.dataChannel?.readyState === 'open';
        });
      } catch (error) {
        console.error('❌ [GET CONNECTED PEERS] Error:', error);
        return [];
      }
    },
    forceReconnect: () => {

// signaling-server-production-FIXED.js - Production server with messaging fix
...
app.get('/signaling-proxy', (req, res) => {
  res.json({
    ...
    endpoint: '/socket.io/',
    version: '1.2.0-messaging-fix',
    features: ['peer-discovery', 'connection-assistance', 'room-management', 'messaging-fix'],
    timestamp: Date.now()
  });
});
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '1.2.0-messaging-fix',
    status: 'running',
    description: 'WebRTC signaling server for peer-to-peer connections with messaging fix',
    endpoints: {
      health: '/health',
      signaling: '/socket.io/'
    },
...
server.listen(PORT, () => {
  console.log(`🎵 PeddleNet Signaling Server v1.2.0-messaging-fix running on port ${PORT}`);
  console.log(`🔍 Health check: http://localhost:${PORT}/health`);
  console.log(`🔔 Features: WebSocket + Chat + Notifications + Room Codes + MESSAGING FIX`);
  console.log(`🎯 CRITICAL FIX: Messages now appear on sending device!`);
});

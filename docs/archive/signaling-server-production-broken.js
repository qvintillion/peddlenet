...
const io = new Server(server, {
  cors: {
    origin: function (origin, callback) {
      ...
      // Regex patterns for complex domains
      ...
      for (const pattern of allowedPatterns) {
        if (pattern.test(origin)) {
          console.log(`✅ CORS: Allowed pattern match: ${origin} matched ${pattern}`);
          return callback(null, true);
        }
      }
      
      // Special case for .web.app domains (Firebase hosting)
      if (origin.endsWith('.web.app')) {
        console.log(`✅ CORS: Allowed .web.app domain: ${origin}`);
        return callback(null, true);
      }
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Check exact matches first
    if (allowedOrigins.includes(origin)) {
      console.log(`✅ Express CORS: Allowed exact origin: ${origin}`);
      return callback(null, true);
    }
    ...
    for (const pattern of allowedPatterns) {
      if (pattern.test(origin)) {
        console.log(`✅ Express CORS: Allowed pattern match: ${origin} matched ${pattern}`);
        return callback(null, true);
      }
    }
    ...
    // Special case for .web.app domains (Firebase hosting)
    if (origin.endsWith('.web.app')) {
      console.log(`✅ Express CORS: Allowed .web.app domain: ${origin}`);
      return callback(null, true);
    }
...
const roomStats = new Map(); // roomId -> { activeUsers, lastActivity, created }
...
app.get('/room-stats/:roomId', (req, res) => {
  ...
  stats.peakUsers = Math.max(stats.peakUsers, activeUsers);
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '1.2.5-messaging-event-fix',
    status: 'running',
    description: 'Enhanced WebRTC signaling server with mobile optimization and Firebase hosting support',
    enhancements: [
...
const connectionRateLimit = {
  perIP: new Map(), // IP -> { count: number, resetTime: number }
  resetInterval: 60000, // 1 minute
  maxConnections: 10, // per IP per minute
  
  canConnect(ip) {
    const now = Date.now();
    const ipData = this.perIP.get(ip) || { count: 0, resetTime: now + this.resetInterval };
    
    // Reset if time window has passed
    if (now > ipData.resetTime) {
      ipData.count = 0;
      ipData.resetTime = now + this.resetInterval;
    }
    
    // Check if under limit
    if (ipData.count >= this.maxConnections) {
      return false;
    }
    
    // Increment and store
    ipData.count++;
    this.perIP.set(ip, ipData);
    return true;
  },
  
  cleanupOldEntries() {
    const now = Date.now();
    for (const [ip, data] of this.perIP.entries()) {
      if (now > data.resetTime + this.resetInterval) {
...
setInterval(() => {
  connectionRateLimit.cleanupOldEntries();
}, 5 * 60 * 1000);
...
io.on('connection', (socket) => {
  ...
  if (!connectionRateLimit.canConnect(clientIP)) {
  ...
  console.log(`🔗 Client connected: ${socket.id} from ${clientIP} (${connectionStats.currentConnections} active)`);
  console.log(`   Transport: ${socket.conn.transport.name}, Upgraded: ${socket.conn.upgraded}`);
  console.log(`   Rate limit status: ${connectionRateLimit.perIP.get(clientIP)?.count || 0}/${connectionRateLimit.maxConnections} connections`);
  ...
  socket.on('join-room', ({ roomId, peerId, displayName }) => {
    try {
      ...
      // Send current peers to new/reconnecting user (as array)
  ...
  // Chat messages with enhanced delivery (FIXED: correct event name)
    ...
    try {
      ...
      if (socket.userData && socket.userData.roomId === roomId) {
        const enhancedMessage = {
          id: message.id || generateDeliveryId(),
        ...
        console.log(`📤 Broadcasting message to room ${roomId}:`, enhancedMessage);
        ...
        console.warn(`⚠️ User ${socket.id} tried to send message to room ${roomId} but is not in that room. Current room: ${socket.userData?.roomId}`);
        socket.emit('error', {
...
server.listen(PORT, () => {
  console.log(`🎵 PeddleNet Enhanced Signaling Server v1.2.5-messaging-event-fix running on port ${PORT}`);
  console.log(`🔍 Health check: http://localhost:${PORT}/health`);
  console.log(`📊 Stability metrics: http://localhost:${PORT}/stability`);
  console.log(`🌐 Enhanced with connection recovery, mobile optimization, and Firebase hosting support`);
